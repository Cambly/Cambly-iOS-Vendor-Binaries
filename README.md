...
| `InstantSearch` (umbrella; product also ships `InstantSearchCore` + `AlgoliaSearchClient` + `InstantSearchInsights` + `InstantSearchTelemetry` + `Logging` + `ISSwiftProtobuf`) | `algolia/instantsearch-ios@7.27.0` (+ transitive `algoliasearch-client-swift`, `instantsearch-telemetry-native`, `apple/swift-log`, `apple/swift-protobuf`) | `instantsearch-7.27.0` | **SPM-mode** — `swift-create-xcframework` on the pure-SwiftPM package (no framework xcodeproj). 7 dynamic frameworks; swift-log patched for library evolution; swift-protobuf renamed → `ISSwiftProtobuf`. See "SPM-mode vendor" below. |
...
- **All 7 modules ship + embed.** `swift-create-xcframework` builds every module — including the transitive dependency packages — as a **separate dynamic framework**; the top-level ones dyld-link the rest at runtime (**not** statically absorbed, same as the Netless stack). So the `InstantSearch` product lists all 7 `.binaryTarget`s — `InstantSearch`, `InstantSearchCore`, `AlgoliaSearchClient`, `InstantSearchInsights`, `InstantSearchTelemetry`, `Logging` (swift-log), `ISSwiftProtobuf` (swift-protobuf, renamed — see below) — and every consuming app target must embed all of them (see "Audit every app target"). Cambly-Swift's two import sites (`import InstantSearch`, `import AlgoliaSearchClient`) both resolve from this single product because `import InstantSearch` `@_exported`-chains through `InstantSearchCore` → `AlgoliaSearchClient`.
- **swift-protobuf renamed → `ISSwiftProtobuf`.** Cambly-Swift's package graph already contains `apple/swift-protobuf` from source (pulled by `Cambly-Analytics-Tags` @ 1.38), so shipping our own `.binaryTarget(name: "SwiftProtobuf")` collides — SwiftPM requires target names to be unique across the whole graph (`multiple packages declare targets with a conflicting name: 'SwiftProtobuf'`). The clean fix (a SwiftPM `moduleAliases` rename) does **not** survive swift-create-xcframework — its generated xcodeproj ignores the alias and the module fails to compile its own internal `SwiftProtobuf.X` self-references. So `postprocess_instantsearch.py` renames it at the Mach-O / bundle level after the build: `SwiftProtobuf.framework` → `ISSwiftProtobuf.framework` (binary, install-id, Info.plist) and repoints every InstantSearch framework's dyld load command. Its `Modules/` is stripped, so it is **embedded but not importable** (no `import ISSwiftProtobuf`) — nobody references this module; it's a pure runtime dep of `InstantSearchTelemetry`. The app keeps its own source `SwiftProtobuf` (1.38, used by Analytics-Tags); the renamed copy coexists in a separate dylib under two-level namespace, so neither the graph collision nor an app-wide protobuf downgrade occurs.
