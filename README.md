...
- **Per-Xcode matrix + binary `.swiftmodule` — NOT a `Package.swift` product.** Unlike every other vendor, InstantSearch is **not** declared in this repo's `Package.swift` and is **not** consumed via SwiftPM. A library-evolution `.swiftinterface` is not stable across Swift compiler versions (a generic open class like `HitsInteractor<Record>`, which Cambly-Swift specializes, fails to re-typecheck under a different compiler), and `swift-create-xcframework` emits these interfaces with `-no-verify-emitted-module-interface` so they don't even round-trip (missing `extension`s, class==module self-refs). So `build-instantsearch.yml` runs a **matrix on `macos-26`** (the only image with every fleet Xcode) building one set of 7 xcframeworks **per Xcode** (`26.2` / `26.3` / `26.4.1` / `26.5`), each xcframework keeping its **binary `.swiftmodule`** (we copy it back after `create-xcframework` strips it). Each leg publishes to a per-Xcode tag `instantsearch-7.27.0-xcode<XV>`. Cambly-Swift consumes them as **local frameworks** — `LocalPackages/InstantSearchBinary/instantsearch-binaries.json` + a render script pick the slice matching the active Xcode (same model as Realm) — so the consumer's compiler always matches the slice and loads the binary `.swiftmodule` directly, never the broken interface.
- **All 7 frameworks ship + embed.** `swift-create-xcframework` builds every module — including the transitive dependency packages — as a **separate dynamic framework**; the top-level ones dyld-link the rest at runtime (**not** statically absorbed, same as the Netless stack). The 7: `InstantSearch`, `InstantSearchCore`, `AlgoliaSearchClient`, `InstantSearchInsights`, `InstantSearchTelemetry`, `Logging` (swift-log), `SwiftProtobuf` (swift-protobuf). Every consuming app target must embed all of them (see "Audit every app target"). Cambly-Swift's two import sites (`import InstantSearch`, `import AlgoliaSearchClient`) both resolve because `import InstantSearch` `@_exported`-chains through `InstantSearchCore` → `AlgoliaSearchClient`; the other 5 are never imported but are dyld-linked at runtime.
- **swift-protobuf: `Modules/` stripped, NOT renamed.** Cambly-Swift's package graph already contains `apple/swift-protobuf` from source (pulled by `Cambly-Analytics-Tags` @ 1.38). `postprocess_instantsearch.py` strips `SwiftProtobuf.framework`'s `Modules/` so it ships as a plain **non-importable** dynamic library — the consumer then sees only ONE importable `SwiftProtobuf` module (its own source 1.38); ours is a pure runtime dyld dep of `InstantSearchTelemetry` (`@rpath/SwiftProtobuf.framework/SwiftProtobuf`). The two `SwiftProtobuf` dylibs (our 1.22 + the app's source 1.38, different install-names) coexist under two-level namespace. We do **not** rename it to `ISSwiftProtobuf`: the rename was only needed when InstantSearch shipped as a `.binaryTarget` (SwiftPM target-name collision), which no longer applies now that it's a local framework — and the rename used `install_name_tool`, which on arm64 left the modified Mach-O's code pages failing runtime code-signing validation (`CODESIGNING Invalid Page` SIGKILL under the debugger). Stripping `Modules/` never touches the binary, so it can't corrupt the signature.
